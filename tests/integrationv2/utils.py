...
def invalid_test_parameters(*args, **kwargs):
    ...
    if protocol is not None and protocol is not Protocols.TLS13:
        if client_certificate is not None and client_certificate.algorithm == 'RSAPSS':
            return True
        if certificate is not None and certificate.algorithm == 'RSAPSS':
            return True
    ...
    if curve is not None:
        if certificate is not None and certificate.compatible_with_curve(curve) is False:
            return True
        if client_certificate is not None and client_certificate.compatible_with_curve(curve) is False:
            return True
