...
extern int s2n_all_valid_handshakes(uint8_t protocol_version, uint32_t *valid_handshakes, int max_valid_handshakes);
...
int main(int argc, char **argv)
{
    ...
    uint32_t valid_tls13_handshakes[100];
    int valid_tls13_handshakes_size;
    GUARD(valid_tls13_handshakes_size = s2n_all_valid_handshakes(S2N_TLS13, valid_tls13_handshakes, 100));
    ...
    /* Test: Use correct state machine for protocol version */
    {
    ...
    /* Test: TLS1.3 server does not wait for client cipher change requests */
    {
        ...
        for (int i=0; i < valid_tls13_handshakes_size; i++) {
            ...
            for (int j=0; j < S2N_MAX_HANDSHAKE_LENGTH; j++) {
                if (s2n_handshake_message(handshake, j, S2N_TLS13) == CLIENT_CHANGE_CIPHER_SPEC) {
    ...
    /* Test: TLS1.3 server does not skip server cipher change requests */
    {
        ...
        for (int i=0; i < valid_tls13_handshakes_size; i++) {
            ...
            for (int j=0; j < S2N_MAX_HANDSHAKE_LENGTH; j++) {
                if (s2n_handshake_message(handshake, j, S2N_TLS13) == SERVER_CHANGE_CIPHER_SPEC) {
                    conn->handshake.message_number = j - 1;
    ...
    /* Test: TLS1.3 s2n_conn_set_handshake_type doesn't set PERFECT_FORWARD_SECRECY or OCSP_STATUS */
    {
        struct s2n_connection *conn = s2n_connection_new(S2N_CLIENT);

        /* Ensure PERFECT_FORWARD_SECRECY is set by choosing a cipher suite with is_ephemeral=1 on the kex */
        ...
        EXPECT_TRUE(conn->handshake.handshake_type & PERFECT_FORWARD_SECRECY );
        EXPECT_TRUE(conn->handshake.handshake_type & OCSP_STATUS );

        /* Verify that tls1.3 DOES NOT set the flags */
        conn->actual_protocol_version = S2N_TLS13;
        EXPECT_SUCCESS(s2n_conn_set_handshake_type(conn));
        EXPECT_FALSE(conn->handshake.handshake_type & PERFECT_FORWARD_SECRECY );
