...
int main(int argc, char **argv)
{
    ...
    /* s2n_signature_algorithms_supported_list_send */
    {
        ...
        /* Test: written signatures readable */
        {
            ...
            EXPECT_OK(s2n_signature_algorithms_supported_list_recv(conn, &result));
            EXPECT_EQUAL(s2n_stuffer_data_available(&result), 0);

            struct s2n_sig_scheme_list *signatures = &conn->handshake_params.server_sig_hash_algs;
    ...
    /* s2n_signature_algorithms_supported_list_process */
    {
        ...
        EXPECT_SUCCESS(s2n_config_add_cert_chain_and_key_to_store(server_config, rsa_pss_cert_chain));
        ...
        /* Test: choose legacy default for <TLS1.2 */
        {
            ...
            /* Test: Client chooses default based on configured cert type */
            {
                ...
                /* Test: ECDSA */
                {
                    ...
                    conn->actual_protocol_version = S2N_TLS11;
                    EXPECT_OK(s2n_signature_algorithms_supported_list_process(conn));
        ...
        /* Test: do not choose invalid schemes */
        {
            /* Test: scheme not valid for higher protocol version */
            {
                ...
                EXPECT_ERROR_WITH_ERRNO(
                        s2n_signature_algorithms_supported_list_process(conn),
            ...
            /* Test: rsa-pss certs not allowed in <TLS1.3 */
            {
                /* No rsa-pss-pss signature schemes for TLS1.2 actually exist.
                 * Create one for testing.
                 */
                struct s2n_signature_scheme pss_pss_tls12_scheme = s2n_rsa_pss_rsae_sha256;
                pss_pss_tls12_scheme.sig_alg = s2n_rsa_pss_pss_sha256.sig_alg;
                const struct s2n_signature_scheme *invalid = &pss_pss_tls12_scheme;
                ...
                /* Succeeds for TLS1.2 and rsa-pss-rsae */
                conn->actual_protocol_version = S2N_TLS12;
                conn->secure->cipher_suite = RSA_CIPHER_SUITE;
                pss_pss_tls12_scheme.sig_alg = s2n_rsa_pss_rsae_sha256.sig_alg;
                EXPECT_OK(s2n_signature_algorithms_supported_list_process(conn));
            };

            /* Test: no certs exist for scheme */
            {
                /* Test one scheme for each cert type */
                const struct s2n_signature_scheme *test_schemes[] = {
                    &s2n_ecdsa_secp384r1_sha384,
                    &s2n_rsa_pss_rsae_sha256,
                    &s2n_rsa_pss_pss_sha256,
                ...
                for (size_t i = 0; i < s2n_array_len(test_schemes); i++) {
                    const struct s2n_signature_scheme *scheme = test_schemes[i];

                    DEFER_CLEANUP(struct s2n_connection *conn = s2n_connection_new(S2N_SERVER),
                            s2n_connection_ptr_free);
                    /* Test with TLS1.3 to avoid needing to match the cipher suite */
                    conn->actual_protocol_version = S2N_TLS13;
                    conn->secure->cipher_suite = TLS13_CIPHER_SUITE;
                    ...
                    /* Succeeds for config that includes all certs */
                    EXPECT_SUCCESS(s2n_connection_set_config(conn, server_config));
                    EXPECT_OK(s2n_signature_algorithms_supported_list_process(conn));
                }
            };
    ...
    /* s2n_signature_algorithms_supported_list_recv */
    {
        ...
        DEFER_CLEANUP(struct s2n_blob list = { 0 }, s2n_free);
        {
            DEFER_CLEANUP(struct s2n_stuffer stuffer = { 0 }, s2n_stuffer_free);
            EXPECT_SUCCESS(s2n_stuffer_growable_alloc(&stuffer, 0));
            struct s2n_stuffer_reservation size = { 0 };
            EXPECT_SUCCESS(s2n_stuffer_reserve_uint16(&stuffer, &size));
            for (size_t i = 0; i < s2n_array_len(test_schemes); i++) {
                EXPECT_SUCCESS(s2n_stuffer_write_uint16(&stuffer, test_schemes[i]->iana_value));
            }
            EXPECT_SUCCESS(s2n_stuffer_write_vector_size(&size));

            EXPECT_SUCCESS(s2n_alloc(&list, s2n_stuffer_data_available(&stuffer)));
            EXPECT_SUCCESS(s2n_stuffer_read(&stuffer, &list));
        }

        /* Test: parse valid client list */
        {
            ...
            EXPECT_SUCCESS(s2n_stuffer_write(&in, &list));
            EXPECT_OK(s2n_signature_algorithms_supported_list_recv(server, &in));

            EXPECT_EQUAL(
                    server->handshake_params.client_sig_hash_algs.len,
                    s2n_array_len(test_schemes));
            for (size_t i = 0; i < s2n_array_len(test_schemes); i++) {
                EXPECT_EQUAL(
                        server->handshake_params.client_sig_hash_algs.iana_list[i],
                        test_schemes[i]->iana_value);
            }
        };

        /* Test: parse valid server list */
        {
            ...
            EXPECT_OK(s2n_signature_algorithms_supported_list_recv(client, &in));
            EXPECT_EQUAL(
                    client->handshake_params.server_sig_hash_algs.len,
                    s2n_array_len(test_schemes));
            for (size_t i = 0; i < s2n_array_len(test_schemes); i++) {
                EXPECT_EQUAL(
                        client->handshake_params.server_sig_hash_algs.iana_list[i],
                        test_schemes[i]->iana_value);
            }
        };
        ...
        /* Test: parse empty list */
        {
            ...
            EXPECT_OK(s2n_signature_algorithms_supported_list_recv(client, &in));
            EXPECT_EQUAL(client->handshake_params.server_sig_hash_algs.len, 0);
        };

        /* Test: parse maximum size list */
        {
            DEFER_CLEANUP(struct s2n_connection *client = s2n_connection_new(S2N_CLIENT),
                    s2n_connection_ptr_free);
            size_t max_size = sizeof(client->handshake_params.server_sig_hash_algs.iana_list);

            struct s2n_stuffer in = { 0 };
            EXPECT_SUCCESS(s2n_stuffer_growable_alloc(&in, 0));
            EXPECT_SUCCESS(s2n_stuffer_write_uint16(&in, max_size));
            EXPECT_SUCCESS(s2n_stuffer_skip_write(&in, max_size));

            EXPECT_OK(s2n_signature_algorithms_supported_list_recv(client, &in));
            EXPECT_EQUAL(
                    client->handshake_params.server_sig_hash_algs.len,
                    TLS_SIGNATURE_SCHEME_LIST_MAX_LEN);
        };

        /* Test: parse malformed list */
        {
            /* Test: list size larger than actual list */
            {
                ...
                EXPECT_SUCCESS(s2n_stuffer_growable_alloc(&in, 0));
                EXPECT_SUCCESS(s2n_stuffer_write(&in, &list));
                EXPECT_SUCCESS(s2n_stuffer_wipe_n(&in, 1));

                EXPECT_OK(s2n_signature_algorithms_supported_list_recv(client, &in));
                EXPECT_EQUAL(client->handshake_params.server_sig_hash_algs.len, 0);
                EXPECT_EQUAL(client->handshake_params.server_sig_hash_algs.iana_list[0], 0);
            };

            /* Test: list too large for storage */
            {
                DEFER_CLEANUP(struct s2n_connection *client = s2n_connection_new(S2N_SERVER),
                        s2n_connection_ptr_free);
                size_t max_size = sizeof(client->handshake_params.server_sig_hash_algs.iana_list);
                size_t bad_size = max_size + 2;

                struct s2n_stuffer in = { 0 };
                EXPECT_SUCCESS(s2n_stuffer_growable_alloc(&in, 0));
                EXPECT_SUCCESS(s2n_stuffer_write_uint16(&in, bad_size));
                EXPECT_SUCCESS(s2n_stuffer_skip_write(&in, bad_size));

                EXPECT_OK(s2n_signature_algorithms_supported_list_recv(client, &in));
                EXPECT_EQUAL(client->handshake_params.server_sig_hash_algs.len, 0);
            };

            /* Test: list size not multiple of 2 */
            {
                DEFER_CLEANUP(struct s2n_connection *client = s2n_connection_new(S2N_SERVER),
                        s2n_connection_ptr_free);
                const size_t odd_size = 5;

                struct s2n_stuffer in = { 0 };
                EXPECT_SUCCESS(s2n_stuffer_growable_alloc(&in, 0));
                EXPECT_SUCCESS(s2n_stuffer_write_uint16(&in, odd_size));
                EXPECT_SUCCESS(s2n_stuffer_skip_write(&in, odd_size));
    ...
    /* Test: libcrypto may not support PSS signatures */
    {
         ...
         * s2n_signature_algorithms_supported_list_process + PSS */
        {
            DEFER_CLEANUP(struct s2n_connection *conn = s2n_connection_new(S2N_CLIENT),
                    s2n_connection_ptr_free);
            conn->secure->cipher_suite = TLS13_CIPHER_SUITE;
