...
int s2n_test_client_auth_message_by_message(bool no_cert)
{
    ...
    EXPECT_HANDSHAKE_TYPE(server_conn, NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH | MIDDLEBOX_COMPAT);
    ...
    EXPECT_HANDSHAKE_TYPE(client_conn, NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH | MIDDLEBOX_COMPAT);
    ...
    if (no_cert) {
        EXPECT_HANDSHAKE_TYPE(client_conn, NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH | NO_CLIENT_CERT | MIDDLEBOX_COMPAT);
    } else {
        EXPECT_HANDSHAKE_TYPE(client_conn, NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH | MIDDLEBOX_COMPAT);
    ...
    if (no_cert) {
        EXPECT_HANDSHAKE_TYPE(server_conn, NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH | NO_CLIENT_CERT | MIDDLEBOX_COMPAT);
    } else {
        EXPECT_HANDSHAKE_TYPE(server_conn, NEGOTIATED | FULL_HANDSHAKE | CLIENT_AUTH | MIDDLEBOX_COMPAT);
