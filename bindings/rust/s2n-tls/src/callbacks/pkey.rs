...
impl PrivateKeyOperation {
    ...
    pub(crate) unsafe fn try_from_cb(
        ...
        Ok(PrivateKeyOperation { raw, kind, source })
    }
...
mod tests {
    ...
    fn ecdsa_sign(op: PrivateKeyOperation, key: &[u8]) -> Result<(), error::Error> {
        match op.kind()? {
    ...
    fn sync_failure() -> Result<(), Error> {
        ...
        match result {
            ...
            Err(e) => error::Error::assert_error(e, ERROR),
        };
        Ok(())
    }
    ...
    fn async_failure() -> Result<(), Error> {
        ...
        impl PrivateKeyCallback for TestPkeyCallback {
            fn handle_operation(
                ...
                self.0.increment();
                let future = TestPkeyFuture { counter: 0, op };
                Ok(Some(Box::pin(future)))
            }
        }
