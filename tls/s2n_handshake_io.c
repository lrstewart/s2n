...
static struct s2n_handshake_action tls13_state_machine[] = {
    ...
    [HELLO_RETRY_REQUEST]       = {TLS_HANDSHAKE, TLS_HELLO_RETRY_REQUEST, 'S', {s2n_client_hello_retry_send, s2n_client_hello_retry_recv}},
...
static const char *message_names[] = {
    ...
    MESSAGE_NAME_ENTRY(HELLO_RETRY_REQUEST),
...
#define PROTOCOL_VERSION( conn )      ((conn)->actual_protocol_version)
#define IS_TLS13_HANDSHAKE( conn )    (PROTOCOL_VERSION(conn) == S2N_TLS13)

#define ACTIVE_STATE_MACHINE( conn )  (IS_TLS13_HANDSHAKE(conn) ? tls13_state_machine : state_machine)

#define ACTIVE_MESSAGE( conn )        s2n_handshake_message((conn)->handshake.handshake_type, (conn)->handshake.message_number, PROTOCOL_VERSION(conn))
#define PREVIOUS_MESSAGE( conn )      s2n_handshake_message((conn)->handshake.handshake_type, (conn)->handshake.message_number - 1, PROTOCOL_VERSION(conn))
