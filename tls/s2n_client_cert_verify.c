...
int s2n_client_cert_verify_recv(struct s2n_connection *conn)
{
    ...
    POSIX_GUARD(s2n_hash_copy(&conn->handshake.temp_hash_copy, &hash_state));

    /* Verify the signature */
    POSIX_GUARD(s2n_pkey_verify(&conn->secure.client_public_key, chosen_sig_scheme.sig_alg, &conn->handshake.temp_hash_copy, &signature));
...
int s2n_client_cert_verify_send(struct s2n_connection *conn)
{
    ...
    S2N_ASYNC_PKEY_SIGN(conn, chosen_sig_scheme.sig_alg, &conn->handshake.temp_hash_copy, s2n_client_cert_verify_send_complete);
}
