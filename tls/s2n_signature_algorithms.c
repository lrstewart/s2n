...
S2N_RESULT s2n_signature_algorithms_supported_list_process(struct s2n_connection *conn)
{
    ...
    for (size_t i = 0; i < signature_preferences->count; i++) {
         ...
         * to use the TLS1.2 defaults even when the client does send the signature_algorithms
...
static S2N_RESULT s2n_signature_algorithms_supported_list_try_recv(
        struct s2n_sig_scheme_list *peer_list, struct s2n_stuffer *in)
{
    RESULT_ENSURE_REF(peer_list);
    ...
    uint16_t pair_count = peer_list_size / 2;
    RESULT_ENSURE_EQ(pair_count * 2, peer_list_size);
    RESULT_ENSURE_LTE(pair_count, TLS_SIGNATURE_SCHEME_LIST_MAX_LEN);

    for (size_t i = 0; i < pair_count; i++) {
        uint16_t sig_scheme = 0;
        RESULT_GUARD_POSIX(s2n_stuffer_read_uint16(in, &sig_scheme));
        peer_list->iana_list[i] = sig_scheme;
    }
    peer_list->len = pair_count;
    return S2N_RESULT_OK;
}
...
{
    struct s2n_sig_scheme_list *peer_list = NULL;
    if (conn->mode == S2N_CLIENT) {
        peer_list = &conn->handshake_params.server_sig_hash_algs;
    } else {
        peer_list = &conn->handshake_params.client_sig_hash_algs;
    ...
    if (s2n_result_is_error(s2n_signature_algorithms_supported_list_try_recv(peer_list, in))) {
        /* Ignore malformed lists. Treat them as missing / zero-length. */
        *peer_list = (const struct s2n_sig_scheme_list) { 0 };
    }
    return S2N_RESULT_OK;
}
