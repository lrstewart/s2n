...
static S2N_RESULT s2n_get_peer_auth_method(struct s2n_connection *conn,
...
static S2N_RESULT s2n_get_legacy_default_signature_scheme(s2n_authentication_method auth_method,
...
S2N_RESULT s2n_signature_algorithm_recv(struct s2n_connection *conn, struct s2n_stuffer *in)
{
    ...
    if (conn->actual_protocol_version < S2N_TLS12) {
        s2n_authentication_method peer_auth_method = 0;
        RESULT_GUARD(s2n_get_peer_auth_method(conn, &peer_auth_method));

        const struct s2n_signature_scheme *default_sig_scheme = NULL;
        RESULT_GUARD(s2n_get_legacy_default_signature_scheme(peer_auth_method,
                &default_sig_scheme));
        RESULT_ENSURE(default_sig_scheme, S2N_ERR_INVALID_SIGNATURE_SCHEME);

        *chosen_sig_scheme = default_sig_scheme;
        return S2N_RESULT_OK;
    }
