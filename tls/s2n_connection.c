...
S2N_RESULT s2n_connection_wipe_all_keyshares(struct s2n_connection *conn)
{
    ...
    GUARD_AS_RESULT(s2n_ecc_evp_params_free(&conn->secure.server_ecc_evp_params));
    for (size_t i = 0; i < S2N_ECC_EVP_SUPPORTED_CURVES_COUNT; i++) {
        GUARD_AS_RESULT(s2n_ecc_evp_params_free(&conn->secure.client_ecc_evp_params[i]));
    }

    GUARD_AS_RESULT(s2n_kem_group_free(&conn->secure.server_kem_group_params));
    for (size_t i = 0; i < S2N_SUPPORTED_KEM_GROUPS_COUNT; i++) {
        GUARD_AS_RESULT(s2n_kem_group_free(&conn->secure.client_kem_group_params[i]));
    }
