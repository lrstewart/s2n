...
static int s2n_evp_hmac_p_hash_digest_init(struct s2n_prf_working_space *ws)
{
    ...
    POSIX_ENSURE_REF(ws->tls.p_hash.evp_hmac.mac_key);
 
    /* Ignore the MD5 check when in FIPS mode to comply with the TLS 1.0 RFC */
    if (s2n_is_in_fips_mode()) {
        POSIX_GUARD(s2n_digest_allow_md5_for_fips(&ws->tls.p_hash.evp_hmac.evp_digest));
    }

    POSIX_GUARD_OSSL(EVP_DigestSignInit(ws->tls.p_hash.evp_hmac.evp_digest.ctx, NULL, ws->tls.p_hash.evp_hmac.evp_digest.md, NULL, ws->tls.p_hash.evp_hmac.mac_key),
...
static int s2n_evp_hmac_p_hash_init(struct s2n_prf_working_space *ws, s2n_hmac_algorithm alg, struct s2n_blob *secret)
{
    ...
    POSIX_ENSURE_REF(ws->tls.p_hash.evp_hmac.mac_key = EVP_PKEY_new_mac_key(EVP_PKEY_HMAC, NULL, secret->data, secret->size));
...
static int s2n_evp_hmac_p_hash_digest(struct s2n_prf_working_space *ws, void *digest, uint32_t size)
{
    ...
    POSIX_GUARD_OSSL(EVP_DigestSignFinal(ws->tls.p_hash.evp_hmac.evp_digest.ctx, (unsigned char *)digest, &digest_size), S2N_ERR_P_HASH_FINAL_FAILED);
                      ...
                      struct s2n_blob *seed_a, struct s2n_blob *seed_b, struct s2n_blob *seed_c, struct s2n_blob *out)
{
    ...
    POSIX_GUARD(hmac->final(ws, ws->tls.digest0, digest_size));
    ...
    while (outputlen) {
        ...
        POSIX_GUARD(hmac->update(ws, ws->tls.digest0, digest_size));
        ...
        POSIX_GUARD(hmac->final(ws, ws->tls.digest1, digest_size));
...
static int s2n_sslv3_finished(struct s2n_connection *conn, uint8_t prefix[4], uint8_t * out)
{
    uint8_t xorpad1[48] =
    ...
    POSIX_GUARD(s2n_hash_copy(&conn->handshake.temp_hash_copy, &conn->handshake.md5));
    {
        struct s2n_hash_state *md5 = &conn->handshake.temp_hash_copy;
    ...
    POSIX_GUARD(s2n_hash_copy(&conn->handshake.temp_hash_copy, &conn->handshake.sha1));
    {
        struct s2n_hash_state *sha1 = &conn->handshake.temp_hash_copy;
...
static int s2n_sslv3_client_finished(struct s2n_connection *conn)
{
    ...
    POSIX_ENSURE_LTE(MD5_DIGEST_LENGTH + SHA_DIGEST_LENGTH, sizeof(conn->handshake.client_finished));
    return s2n_sslv3_finished(conn, prefix, conn->handshake.client_finished);
}
...
static int s2n_sslv3_server_finished(struct s2n_connection *conn)
{
    ...
    return s2n_sslv3_finished(conn, prefix, conn->handshake.server_finished);
}
...
int s2n_prf_client_finished(struct s2n_connection *conn)
{
    ...
    if (conn->actual_protocol_version == S2N_TLS12) {
        ...
        case S2N_HMAC_SHA256:
            POSIX_GUARD(s2n_hash_copy(&conn->handshake.temp_hash_copy, &conn->handshake.sha256));
            POSIX_GUARD(s2n_hash_digest(&conn->handshake.temp_hash_copy, sha_digest, SHA256_DIGEST_LENGTH));
            sha.size = SHA256_DIGEST_LENGTH;
            break;
        case S2N_HMAC_SHA384:
            POSIX_GUARD(s2n_hash_copy(&conn->handshake.temp_hash_copy, &conn->handshake.sha384));
            POSIX_GUARD(s2n_hash_digest(&conn->handshake.temp_hash_copy, sha_digest, SHA384_DIGEST_LENGTH));
    ...
    POSIX_GUARD(s2n_hash_copy(&conn->handshake.temp_hash_copy, &conn->handshake.md5));
    POSIX_GUARD(s2n_hash_digest(&conn->handshake.temp_hash_copy, md5_digest, MD5_DIGEST_LENGTH));
    md5.data = md5_digest;
    md5.size = MD5_DIGEST_LENGTH;

    POSIX_GUARD(s2n_hash_copy(&conn->handshake.temp_hash_copy, &conn->handshake.sha1));
    POSIX_GUARD(s2n_hash_digest(&conn->handshake.temp_hash_copy, sha_digest, SHA_DIGEST_LENGTH));
    sha.data = sha_digest;
