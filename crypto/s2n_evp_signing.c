...
{
    /* EVP hashes are only required when using the "digest-and-sign" version
     * of the EVP signing logic, which is currently only used for awslc-fips.
     * See s2n_evp_digest_and_sign.
     *
     * EVP hashes for FIPS-approved hashing algorithms will also be required
     * for openssl-3.0-fips, but that is not yet supported.
     *
     * All other use cases can use EVP hashes, but are not required to.
     * For now, we default to not using EVP hashes. This is simpler because
     * some libcryptos, like openssl-1.0.2 and openssl-3.0-fips, do not
     * support EVP hashes for some hash algorithms, like MD5+SHA1.
     */
        ...
        struct s2n_hash_state *hash_state, struct s2n_blob *signature)
{
    ...
    POSIX_ENSURE(s2n_evp_signing_requires_evp_hash(), S2N_ERR_HASH_NOT_READY);
        ...
        struct s2n_hash_state *hash_state, struct s2n_blob *signature)
{
    ...
    POSIX_GUARD_OSSL(EVP_PKEY_sign(pctx, signature->data, &signature_size,
            digest_out, digest_length), S2N_ERR_SIGN);
    POSIX_ENSURE(signature_size <= signature->size, S2N_ERR_SIZE_MISMATCH);
    signature->size = signature_size;
